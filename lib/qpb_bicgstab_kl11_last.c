...
static int precond, n_echo, restart;
...
qpb_bicgstab_kl11_last_init(void * gauge, qpb_clover_term clover, 
			    ...
			    int precondition, qpb_double eps_prec, 
		       ...
		       qpb_double epsilon, int max_iter)
{
  ...
  if(precond) {
    ...
    qpb_double kappa = 1./(8.0+2.0*m);
    qpb_spinor_xeqy(r, b_tilde);
    qpb_bicgstab(b_tilde, r, ov_params.gauge_ptr, ov_params.clover, kappa, ov_params.c_sw, 
		 precond_eps, max_iter);
  }
  ...
  if(precond) {
    ...
    if(x_norm != 0) {
      Op(y, x);
      qpb_bicgstab(r, y, ov_params.gauge_ptr, ov_params.clover, kappa, ov_params.c_sw, 
		   precond_eps, max_iter);
    } else {
  ...
  for(iters=1; iters<max_iter; iters++)
	...
	qpb_double kappa = 1./(8.0+2.0*m);
	Op(y, p);
	qpb_bicgstab(q, y, ov_params.gauge_ptr, ov_params.clover, kappa, ov_params.c_sw, 
		     precond_eps, max_iter);
      }else{
	Op(q, p);
      }
      ...
      if(iters % n_reeval == 0)
	{
	  if(precond) {
	    ...
	    qpb_double kappa = 1./(8.0+2.0*m);
	    Op(z, x);
	    qpb_bicgstab(r, z, ov_params.gauge_ptr, ov_params.clover, kappa, ov_params.c_sw, 
			 precond_eps, max_iter);
	    qpb_spinor_xmy(r, b_tilde, r);
	  } else {
	    Op(r, x);
