...
void
qpb_nucleon_2x2_2pt(qpb_complex **corr_x, qpb_spinor_field *spinor0, qpb_spinor_field *spinor1)
{
...
#	pragma omp parallel for
  ...
  for(int t=0; t<lt; t++)
	...
	qpb_complex Cg5q1C[NS*NC][NS*NC];
	qpb_complex Cq1Cg5[NS*NC][NS*NC];
	...
	for(int i=0; i<4; i++)
	  {
	    qpb_complex A[NS][NS];
	    qpb_complex B[NS][NS];
	    qpb_complex Contr1[NS*NC][NS*NC];
	    qpb_complex Contr2[NS*NC][NS*NC];
	    ...
	    switch(i)
	      {
	      case 0:
		prop_contract_02(Contr1, Cg5q1Cg5, q0);
		for(int a0=0; a0<NC; a0++)
		  for(int a1=0; a1<NC; a1++)
			...
			for(int ku=0; ku<NS; ku++)
			  {
			    /* N - to - N */
			    A[mu][nu].re += CMULR( q0[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
			    A[mu][nu].im += CMULI( q0[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
			    
			    B[mu][nu].re += CMULR( q0[IDX(a0, mu)][IDX(a1, ku)] , Contr1[IDX(a1, ku)][IDX(a0, nu)] );
			    B[mu][nu].im += CMULI( q0[IDX(a0, mu)][IDX(a1, ku)] , Contr1[IDX(a1, ku)][IDX(a0, nu)] );
			  }
		break;
	      case 1:
		prop_contract_13(Contr1, Cg5q1C, q0);
		for(int a0=0; a0<NC; a0++)
		  for(int a1=0; a1<NC; a1++)
		    for(int mu=0; mu<NS; mu++)
		      for(int nu=0; nu<NS; nu++)
			for(int ku=0; ku<NS; ku++)
			  {
			    A[mu][nu].re += CMULR( q0g5[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
			    A[mu][nu].im += CMULI( q0g5[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
		    
			    B[mu][nu].re += CMULR( q0g5[IDX(a0, ku)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, mu)] );
			    B[mu][nu].im += CMULI( q0g5[IDX(a0, ku)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, mu)] );
			  }
		break;
	      case 2:
		prop_contract_02(Contr1, Cq1Cg5, q0);
		for(int a0=0; a0<NC; a0++)
		  for(int a1=0; a1<NC; a1++)
		    for(int mu=0; mu<NS; mu++)
		      for(int nu=0; nu<NS; nu++)
			for(int ku=0; ku<NS; ku++)
			  {
			    A[mu][nu].re += CMULR( g5q0[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
			    A[mu][nu].im += CMULI( g5q0[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
			    
			    B[mu][nu].re += CMULR( g5q0[IDX(a0, mu)][IDX(a1, ku)] , Contr1[IDX(a1, ku)][IDX(a0, nu)] );
			    B[mu][nu].im += CMULI( g5q0[IDX(a0, mu)][IDX(a1, ku)] , Contr1[IDX(a1, ku)][IDX(a0, nu)] );
			  }
		break;
	      case 3:				
		prop_contract_02(Contr1, Cq1C, q0);
		prop_contract_13(Contr2, Cq1C, g5q0);
		for(int a0=0; a0<NC; a0++)
		  for(int a1=0; a1<NC; a1++)
		    for(int mu=0; mu<NS; mu++)
		      for(int nu=0; nu<NS; nu++)
			for(int ku=0; ku<NS; ku++)
			  {
			    A[mu][nu].re += CMULR( g5q0g5[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
			    A[mu][nu].im += CMULI( g5q0g5[IDX(a0, mu)][IDX(a1, nu)] , Contr1[IDX(a1, ku)][IDX(a0, ku)] );
			    
			    B[mu][nu].re += CMULR( q0g5[IDX(a0, ku)][IDX(a1, nu)] , Contr2[IDX(a1, ku)][IDX(a0, mu)] );
			    B[mu][nu].im += CMULI( q0g5[IDX(a0, ku)][IDX(a1, nu)] , Contr2[IDX(a1, ku)][IDX(a0, mu)] );
			  }
	    ...
	    for(int mu=0; mu<NS; mu++)
	      for(int nu=0; nu<NS; nu++)
		  corr_x[i*16*lt + (mu*NS+nu)*lt + t][lv] = CADD(A[mu][nu], B[mu][nu]);		  
	  }
      }
  return;
}
...
/* void */
/* _qpb_nucleon_star_2pt(qpb_complex **corr_x, qpb_spinor_field *spinor0, qpb_spinor_field *spinor1) */
/* { */
/*   int lvol = problem_params.l_vol; */
/*   int lt = problem_params.l_dim[0]; */
/*   int lvol3d = lvol/lt; */
