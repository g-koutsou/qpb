...
void
qpb_tetraq_mol_displ_2pt(qpb_complex **corr_x[2][2], qpb_spinor_field *spinor[2][2])
{
	  ...
	  for(int j=0; j<2; j++)
	    {
	      ...
	      prop_G_dag(q1, q0[i][j]);
	      prop_G_G(GGyy, q0[i][j], q1);

	      prop_G_dag(q1, q0[0][j]);
	      prop_G_G(GGxy, q0[0][j], q1);

	      prop_G_dag(q1, q0[i][0]);
	      prop_G_G(GGyx, q0[i][0], q1);

	      prop_G_dag(q1, q0[i][0]);
	      prop_G_G(GxxGyx, q0[0][0], q1);

	      prop_G_dag(q1, q0[0][j]);
	      prop_G_G(GyyGxy, q0[i][j], q1);
		...
		for(int mu=0; mu<NS; mu++)
	      ...
	      corr_x[i][j][0*lt+t][lv] = B;
	      corr_x[i][j][1*lt+t][lv] = A;
	    }
      }
  return;
}
