...
void
qpb_write_gauge_64(qpb_gauge_field gauge_field, char fname[])
{
  MPI_Datatype mpi_dtype_link_float, filetype;
  MPI_Type_contiguous(2*NC*NC*ND, MPI_FLOAT, &mpi_dtype_link_float);
  MPI_Type_commit(&mpi_dtype_link_float);
  ...
  void *buffer = qpb_alloc(problem_params.l_vol*ND*sizeof(qpb_link_double));
  
  for(int v=0; v<problem_params.l_vol; v++)
    for(int mu=0; mu<ND; mu++)
      for(int col=0; col<NC*NC; col++)
	{
	  ((double *) buffer)[v*ND*NC*NC*2 + mu*NC*NC*2 + col*2] = gauge_field.bulk[v][ND-1-mu][col].re;
	  ((double *) buffer)[v*ND*NC*NC*2 + mu*NC*NC*2 + col*2 + 1] = gauge_field.bulk[v][ND-1-mu][col].im;
	}

  if(!qpb_is_bigendian())
    qpb_byte_swap_double(buffer, problem_params.l_vol*NC*NC*ND*2);
