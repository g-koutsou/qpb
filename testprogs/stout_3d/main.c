...
main(int argc, char *argv[])
{
  ...
  qpb_double stout_rho;
  if(sscanf(qpb_parse("Stout rho"), "%lf", &stout_rho)!=1)
    {
      error("error parsing for %s\n",
	    "Stout rho");
      exit(QPB_PARSER_ERROR);
  ...
  int stout_niter;
  if(sscanf(qpb_parse("Stout iterations"), "%d", &stout_niter)!=1)
    {
      error("error parsing for %s\n",
	    "Smear iterations");
      exit(QPB_PARSER_ERROR);
  ...
  int n_stout_3d;
  qpb_double rho_stout_3d;
  if(sscanf(qpb_parse("Stout 3D iterations"), "%d", &n_stout_3d)!=1)
    {
      error("error parsing for %s\n", 
	    "Stout 3D iterations");
      exit(QPB_PARSER_ERROR);	  
  ...
  if(sscanf(qpb_parse("Stout 3D rho"), "%lf", &rho_stout_3d)!=1)
    {
      error("error parsing for %s\n", 
	    "Stout 3D rho");
      exit(QPB_PARSER_ERROR);	  
...
#pragma omp parallel
  ...
  print(" stout rho = %g\n", stout_rho);
  print(" stout iterations = %d\n", stout_niter);
  print(" stout 3D smearing = (%f, %d)\n", rho_stout_3d, n_stout_3d);
  ...
  /* stout smear the gauge field */
  if(stout_niter != 0)
    {
      qpb_gauge_field stoutgauge = qpb_gauge_field_init();

      print(" stout smear gauge field...\n");
      qpb_stoutsmear_niter(stoutgauge, gauge, stout_rho, stout_niter);
      qpb_gauge_field_copy(gauge, stoutgauge);
      qpb_gauge_field_finalize(stoutgauge);
  ...
  if(n_stout_3d != 0)
    {
      print(" 3D-stout smear gauge field...\n");
      for(int iter=0; iter<n_stout_3d; iter++)
	{
	  qpb_stoutsmear_3d_niter(gauge_aux[(iter+1)%2], gauge_aux[iter%2], rho_stout_3d, 1);
